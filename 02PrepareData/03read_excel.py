...
'''
read_excel()
: 엑셀을 데이터프레임으로 변환. 첫행은 타이틀로 인식하므로 header=0이 디폴트 값이다.
...
'''
header=1은 인덱스1 즉, 1행을 타이틀로 간주하겠다는 의미이므로 2행이 타이틀로 인식되어 데이터프레임으로
변환한다.'''
...
'''
데이터를 가져오는 것은 df1과 동일하지만 타이틀이 없는 것으로
간주되어 정수형 컬럼명이 타이틀로 지정된다.'''
